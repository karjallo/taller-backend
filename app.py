...
app.config["SQUALCHEMY_DATABASE_URI"] = f"sqlite:///{DB_PATH}"
...
@app.route("/")
def index():
    #buscar el rl, el numero de la pagina con un metodo GET
    ...
    if name:
        ...
        # 200 es si todo esta bien
        if response.status_code != 200:
            return render_template("index.html", character=[],
            search=True, error_message='Personaje No encontrado')
        data = response.json()
    ...
    return render_template("index.html", characters=data["results"],
    info=data['info'], page=int(page), search=False)
